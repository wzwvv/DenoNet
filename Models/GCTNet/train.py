    ...
    for epoch in range(opts.epochs):
        ...
        for batch_id in trange(train_data.len()):
            ...
            if batch_id % 1 == 0:
                ...
                fake_y,fake_cls,_, _, _ = model_d(p_t.unsqueeze(dim=1))
                real_y,real_cls,_, _, _ = model_d(y_t.unsqueeze(dim=1))
            ...
            if batch_id % 1 == 0:
                ...
                fake_y,fake_cls, _, fake_feature2, _ = model_d(p_t.unsqueeze(dim=1))
                _, real_cls, _, true_feature2, _ = model_d(y_t.unsqueeze(dim=1))
                ce = torch.nn.CrossEntropyLoss()

                L_feat = ce(real_cls, y_t) + ce(fake_cls, y_t)
